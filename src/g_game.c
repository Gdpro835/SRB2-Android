...
#define CAMCVARFLAGS (CV_FLOAT|CV_SAVE|CV_SLIDER_SAFE|CV_ALLOWLUA)
...
boolean G_Responder(event_t *ev)
{
		...
		((demoplayback && !modeattacking && !titledemo) || gamestate == GS_TITLESCREEN))
	{
		...
		&& !(gamestate == GS_TITLESCREEN && finalecount < (cv_tutorialprompt.value ? TICRATE : 0)))
		{
#ifdef TOUCHINPUTS
			if (finger)
...
void G_LoadGameData(gamedata_t *data)
{
	...
	if (!length)
	{
		length = FIL_ReadFile(va(pandf, srb2path, gamedatafilename), &savebuffer);
		if (length != 0)
			gamedatainpath = true;
	}
#endif
